"""Functions for currency calculator."""


def exchange_money(budget: float, exchange_rate: float) -> float:
    """Calculate value of the exchanged currency.
...
def get_change(budget: float, exchanging_value: int) -> float:
    """Calculate the amount of money that is lef from the budget.
...
def exchangeable_value(
    ...
    """Calculate the maximum value after exchange.
    ...
    exchanged_value = exchange_money(budget, rate_with_spread)
...
def non_exchangeable_value(
    ...
    """Calculate the non-exchangeable value.
    ...
    rate_with_spread = exchange_rate * (1 + spread / 100)
    exchanged_value = exchange_money(budget, rate_with_spread)
    exchangeable_val = exchangeable_value(budget, exchange_rate, spread, denomination)

    return int(exchanged_value - exchangeable_val)
